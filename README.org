...
** Table of contents :TOC:
  - [[#m-x][M-x]]
  - [[#m-x-rofi-or-any-other-menu-utility][M-x-rofi (or any other 'menu' utility)]]
  - [[#leader][Leader]]
...
** M-x-rofi (or any other 'menu' utility)
Essentially same as the =M-x=, but instead of using =extended-menu= module uses
external program ([[https://github.com/davatorium/rofi][rofi]] by default) to display and search through commands.

_Setup_:
- copy =M-x-rofi.lua= to your =~~/scripts=
- copy =./script-opts/M_x_rofi.conf= to your =~~/scripts-opts=

See config, there you can redefine which executable is used and define its flags
